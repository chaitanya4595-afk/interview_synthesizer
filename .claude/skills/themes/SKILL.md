...
3. Write each theme's headline first, then mark every claim in it For or Against that headline. A claim is Against if, read at face value, it weakens or contradicts the headline. Do not reword the headline to absorb an Against claim. Count each interview once per theme, on the side of its overall position in that theme. Label it Mixed if it is genuinely split, and show Mixed separately from Support and Against. Each theme with Against > 0 must include at least one Against quote.
...
Support: N interviews [INTxx, INTxx] | Mixed: N [INTxx] | Against: N [INTxx]
Key quotes:
* "<exact quote>" [INTxx]
...
* Keep every real disagreement in the disagreements section, even if it doesn't map to an Against count. Never remove a disagreement to satisfy a counting rule.